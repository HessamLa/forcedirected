...
class FDModel(ForceDirected):
    ...
    DESCRIPTION=f"Same as the shell model (v204). "\
        ...
        "The set of landmark nodes is the set node in the reach_max-th neighborhood, and a fixed number of landmark nodes chosen randomly."
    def __init__(self, Gx, n_dim,
                tlog_coeff:int=100, random_nodes_ratio:list=[0.5, 0.25],
                reach_a:int=1, reach_r:int=2,
        ...
        reach_a, reach_r, tlog_coeff = int(reach_a), int(reach_r), int(tlog_coeff)
        reach_max = max(reach_a, reach_r)
        ...
        # For each row and its equivalent column (its symmetric), only keep the values {0,1, 2,..., reach_max} as well as a
        # maximum number of t values not in {0, 1, ... reach_max}. Zero out the rest. 
        # This is to keep the landmark nodes in reach_max-th neighborhood, and a fixed number of nodes chosen randomly.
        ...
        def update_hops_matrix_3(H, reach_max, max_random_count, p: list = [0.50, 0.25]):
            n = H.shape[0]  # number of nodes
            Hupdate = np.zeros_like(H, dtype=float)
            h_counts = np.zeros(n, dtype=int)
            ...
            for i in range(n):
                # Nodes within reach_max-hops
                within_reach_max = set(np.where((H[i] <= reach_max) & (H[i] > 0))[0])
                
                # Categorize nodes beyond reach_max-hops
                beyond_reach_max_categories = [set(np.where(H[i] == reach_max + j + 1)[0]) for j in range(num_categories - 1)]
                beyond_reach_max_categories.append(set(np.where(H[i] > reach_max + num_categories - 1)[0]))
                
                # Randomly select nodes from each category
                selected_beyond_reach_max = set()
                for category, k in zip(beyond_reach_max_categories, k_count):
                    selected_beyond_reach_max.update(random.sample(category, min(k, len(category))))
                
                # Combine the sets
                reachable = within_reach_max.union(selected_beyond_reach_max)
                
                # Update Hupdate matrix
                for j in reachable:
                    Hupdate[i, j] = H[i, j]
                
                # Count entries in range (0, reach_max]
                h_counts[i] = np.sum((Hupdate[i, :] > 0) & (Hupdate[i, :] <= reach_max))
            
            return Hupdate, h_counts        
        
        t = tlog_coeff*int(np.log(self.n_nodes))# keep the maximum of t values not in the intrange [0, reach_max] 
        print(f"reach_max={reach_max}, number of random nodes beyond reach_max={t}")
        hops, _ = update_hops_matrix_3(hops, reach_max, t, p=random_nodes_ratio)
        ...
        #     # print(f"{k:<20s}: {sys.getsizeof(v)} bytes")
        #     print(f"{k:<20s}: {asizeof.asizeof(v)} bytes")
        # print(f"{'self.hops':<20s}: {self.hops.element_size() * self.hops.nelement()/2**20} Mbytes")
        # print(f"{'self.Z':<20s}: {self.Z.element_size() * self.Z.nelement()/2**20} Mbytes")
        if(self.verbosity>=3):
    ...
    def forward(self, bmask, **kwargs):
        ...
        # print(f"{'H':<20s}: {H.element_size() * H.nelement()/2**20} Mbytes")
        # print(f"{'D':<20s}: {D.element_size() * D.nelement()/2**20} Mbytes")
        # print(f"{'N':<20s}: {N.element_size() * N.nelement()/2**20} Mbytes")
        # print(f"{'sh_avg_coeff':<20s}: {sh_avg_coeff.element_size() * N.nelement()/2**20} Mbytes")
        ...
        Fa = torch.where(mask, k1 * sh_avg_coeff * N * torch.exp(-k2 * (H - 1)), torch.zeros_like(H))
        ...
        Fr = torch.where(mask, -k3 * H * torch.exp(-N*k4), torch.zeros_like(H))
        F = Fa + Fr
