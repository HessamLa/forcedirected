...
class FDModel(ForceDirected):
    ...
    VER_MIN="02"
    DESCRIPTION=f"The basic FD model: f(x) = ln(k1 * x / h) . Derives from the class ForceDirected"
    ...
    def forward(self, bmask, **kwargs):
        ...
        F = torch.where(H>0, torch.log(k1*(NN)/HH), torch.zeros_like(H))
