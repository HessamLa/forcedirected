...
from forcedirected.Functions import attractive_force_base
...
class FDModel(Model_Base):
    ...
    VERSION="0111nodrop"
    DESCRIPTION="with base forces, alpha={alpha}"
    def __str__(self):
    ...
    def __init__(self, Gx, n_dim, alpha:float,
        ...
        self.hops = torch.tensor(self.hops)
        self.e_hops = torch.exp(-self.hops+1) # e^(-(h-1)
        self.e_hops[self.hops==0]=0


        self.fmodel_attr = ForceClass(name='attractive_force_base', 
                            func=lambda *args, **kwargs: attractive_force_base(*args, k1=1.0/self.n_nodes, k2=1, **kwargs)
                            )
        self.fmodel_repl = ForceClass(name='repulsive_force_hops_exp',
                            ...
                            func=lambda *args, **kwargs: repulsive_force_hops_exp(*args, k1=10/self.n_nodes, k2=0.9, **kwargs)
                            )
    ...
    def train(self, epochs=100, device='cpu', row_batch_size='auto', **kwargs):
        ...
        self.e_hops = torch.tensor(self.e_hops).to(device)
        ...
        def run_batches(batch_count=1, **kwargs):
            ...
            for i, bmask in enumerate (batchify(list(range(self.Z.shape[0])), batch_size=batch_size)):
                ...
                self.forcev.F1.v[bmask] = self.fmodel_attr(D, N, unitD, e_hops=self.e_hops[bmask,:]) # pass the current model (with its contained embeddings) to calculate the force
