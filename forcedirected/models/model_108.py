...
class FDModel(Model_Base):
    ...
    VERSION="0108"
    DESCRIPTION="Same as 104. With biased dimension. Exponential Decay repulsive(k1=10, k2=0.9), alpha={alpha}"
    def __str__(self):
    ...
    def train(self, epochs=100, device='cpu', row_batch_size='auto', **kwargs):
        ...
        def run_batches(batch_count=1, **kwargs):
            ...
            for i, bmask in enumerate (batchify(list(range(self.Z.shape[0])), batch_size=batch_size)):
                ...
                # dim_bias = np.array([1+i*4//self.n_dims for i in range(self.n_dims)]).reshape(1, 1, -1) 
                # dim_bias = np.repeat(dim_bias, D.shape[0], axis=0)
                # dim_bias = np.repeat(dim_bias, D.shape[1], axis=1)

                mat_dim_bias = self.dim_bias.repeat(D.shape[0], D.shape[1], 1)
                mat_dim_bias = mat_dim_bias<=hops[...,None]
